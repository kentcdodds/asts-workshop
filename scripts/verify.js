...
var errors = {
  ...
  oldYarn: {
    getMessage: function(desired, actual) {
      return 'Your version of yarn (' +
        actual +
        ') is older than the recommended version of ' +
        desired +
        '. ' +
...
var systemErrors = Object.keys(errors).filter(function(key) {
  return errors[key].isProblem
})
...
function versionIsGreater(desiredVersion, actualVersion) {
  var desiredVersionsNumbers = /v?(\d+)\.(\d+)\.(\d+)/.exec(desiredVersion)
  var desiredMajor = Number(desiredVersionsNumbers[1])
  var desiredMinor = Number(desiredVersionsNumbers[2])
  var desiredPatch = Number(desiredVersionsNumbers[3])
